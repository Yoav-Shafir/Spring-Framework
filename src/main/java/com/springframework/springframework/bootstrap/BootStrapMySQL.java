...
// This code runs only when profiles: dev or prod are active.
// Its just a way of checking if we have any missing values, if so,
// we add them to the database.

@Slf4j
@Component
// This profile will pick the `application-dev.yml` or `application-prod.yml` configuration file.
// This profile uses the MySQL database.
@Profile({"dev", "prod"})
